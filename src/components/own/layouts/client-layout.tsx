...
export default function MainLayout() {
    ...
    return (
        <div className="min-h-screen flex flex-col bg-white">
            <AdminFloatingButton />
            <PageHeader />
            <nav className="border-t border-white/10 bg-primary/80">
...
const HeaderLink = ({ to, children }: { to: string, children?: any }) => {
    return (
        <NavLink
            ...
            to={to}
            className={({ isActive }) =>
                `relative inline-flex items-center gap-1 text-sm font-semibold transition ${isActive ? 'text-blue-500' : 'text-slate-300 hover:text-white'
                } after:absolute after:left-0 after:-bottom-1 after:h-[2px] after:w-0 after:bg-blue-500 after:transition-all after:duration-300 hover:after:w-full ${isActive ? 'after:w-full' : ''
                }`
            }>
